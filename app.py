...
# Load assets defensively
MODEL_PATH = os.path.join("pickle files", "LogisticR_heart.pkl")
SCALER_PATH = os.path.join("pickle files", "scaler.pkl")
COLS_PATH = os.path.join("pickle files", "columns.pkl")
...
try:
    ...
    missing_assets.append(os.path.basename(MODEL_PATH))

try:
    scaler = joblib.load(SCALER_PATH)
except Exception as e:
    missing_assets.append(os.path.basename(SCALER_PATH))

try:
    expected_columns = joblib.load(COLS_PATH)
except Exception as e:
    missing_assets.append(os.path.basename(COLS_PATH))

ASSETS_OK = (model is not None) and (scaler is not None) and (expected_columns is not None)
...
with predict_tab:
    if not ASSETS_OK:
        st.error("Model artifacts are missing or could not be loaded. Predict is disabled.")
        st.markdown("**Missing files:** " + ", ".join(missing_assets))
        st.info("Place the missing files in the `pickle files/` folder and reload the app.")
    else:
        # --- Form ---
...
with report_tab:
    ...
    if df is None:
        ...
        # Essential visualizations (require plotly)
        if not PLOTLY_AVAILABLE:
            st.info("Plotly is not installed — report visualizations are disabled. Install with: pip install plotly")
...
# End of file (removed duplicated form/prediction block that caused incorrect behavior)